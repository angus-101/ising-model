...
cdef energy_diff(long[:, :, :] cells, double inverse_temp, int threads):
    
    # Iterates over all the cells in the array, carrying out the energy
    # calculation on each one
    # Uses prange in the outermost loop to implement OpenMP
    ...
    for row in prange(z_size, nogil=True, num_threads=threads):
