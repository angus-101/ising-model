...
def create_share(z_size):
    ...
    z_small = int(z_size / numtasks)
    last_processor = numtasks - 1
    cells = create_cells(z_small, z_size, z_size)
    top = divide(cells)[0]
...
def mpi_ising(z_size, inverse_temp, iterations):
...
def mpi_main(z_size, inverse_temp, iterations):
...
def create_cells(z_size, y_size, x_size):
    
    # Creates array of given size filled with random distribution of
    # either +1 or -1
    
    cells = np.random.choice([-1, 1], size = (z_size, y_size, x_size))
...
def main(array_size, inverse_temp, iterations):
    ...
    # Does everything, times it, and produces an image
